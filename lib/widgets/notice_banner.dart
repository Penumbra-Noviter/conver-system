...
/// - 动效（M6-07 克制动效 ②）：挂载出场淡入 140ms（消费 `ConverDurations.fast`
///   token）；出现/消失过渡内建于组件自身，父级条件渲染负责挂载时机
///   （`AnimatedSwitcher` 式的进出动画在挂载侧收敛，组件本体零外部改动）。
library;
...
class NoticeBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<double>(
      // 挂载出场淡入 140ms（M6-07；消费 token 不硬编码）。
