...
List<LlmMessage> buildEvolutionMessages({
  ...
  return [
    LlmMessage(
      role: 'system',
      content: '你是「$charName」的人设演化助手。基于当前人格设定与对话中沉淀的'
...
///
/// [model] 为调用模型名（透传 provider）；返回 [Future<String>] 与
/// [PersonaReflector] 签名同形（装配层闭包绑定 llm/model 后传入服务）。
...
class PersonaEvolutionService {
  ...
  Future<bool> applyRevision(int revisionId) async {
    ...
    await _characterRepository.updateCharacter(
      ...
      CharactersCompanion(
        personality: Value(revision.personalitySnapshot),
      ),
    );
    return true;
  }
