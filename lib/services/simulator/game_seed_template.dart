/// Conver System — 叙事游戏种子模板（F-M5-08a）。
///
/// 桌面权威源（只读，语义逐字锚点）：`desktop/backend/app/services/game_template.py`
/// （SEED_TEMPLATE 逐字 / MARKER_PATTERN 逐字）。完整、自包含的 HTML 叙事选择
/// 游戏模板；LLM 通过替换两个模板标记（`<!-- GEN:config -->` 与
/// `<!-- GEN:scenes -->`）来填充游戏数据，生成可运行的 HTML 模拟器游戏。
///
/// 模板标记契约：
/// - `<!-- GEN:config -->` → JSON 对象：`{"title": 游戏标题, "world": 世界观简介}`；
/// - `<!-- GEN:scenes -->` → JSON 数组：每个场景 `{"id", "narrative", "choices"}`，
///   每个选项 `{"text", "next"}`；空 choices 数组 → 终局场景。
///
/// 校验闸门（`generated_game_validator.dart`）使用本模块的 [markerPattern] 检测
/// 模板标记是否全部替换完毕。
library;

/// 种子模板 + 标记正则 + cfg- 三元组契约——生成域单源。
///
/// [cfgRequiredIds] 与桌面 `game_template.py` 内嵌 cfg- 三元组 + 导入侧
/// `simulator_import.CFG_REQUIRED_IDS` 语义同构（key-injector 探测锚点）；本域
/// 契约单一归属，校验闸门经本常量消费，不复制字面量。
abstract final class GameSeedTemplate {
  /// 模板标记正则（校验闸门检测替换完整性；桌面 MARKER_PATTERN
  /// `r"<!--\s*GEN:"` + IGNORECASE 逐字）。
  static final RegExp markerPattern =
      RegExp(r'<!--\s*GEN:', caseSensitive: false);

  /// cfg- 契约所需三元组 input id（key-injector 探测锚；生成域单源）。
  static const Set<String> cfgRequiredIds = <String>{
    'cfg-endpoint',
    'cfg-apikey',
    'cfg-model',
  };

  /// 种子模板 HTML（LLM 在此填充数据；完整自包含，零外部依赖。桌面
  /// SEED_TEMPLATE 逐字）。
  static const String seedTemplate = r"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
