...
## 技术债消费批次 F-157 — AboutPage 版本防漂移（2026-09-22 — 用户「顺手修掉吧」拍板）

- **批次源**：模拟器简介批次期末全量发现（`manual_pages_test` 版本防漂移基线即挂——pubspec 已 `1.1.0+2`、`AboutPage.appVersion` 仍 `1.0.0+1`，发布批 7f20557 改版本号漏改 About 页；`git stash` 回基线复现确认非本批引入）；用户「顺手修掉吧」拍板消费。
- **修复**：`about_page.dart:22` 常量对齐 `'1.1.0+2'`（docstring 更新，注明发版改号须与 pubspec 同步 + 指向发布手册）；**根治复发**——`release-android.md` §1「修改位置」固化**双同步点**（pubspec version 行 + AboutPage.appVersion 常量，漏改即防漂移测试红，1.1.0 发布漏改实锤注记）。
- **门禁**：`manual_pages_test` 版本防漂移转绿（11/11）；全量 **2898 测**绿（基线 2897 + 存量 1 失败清零）/ analyze 0。
- **批次收尾**：TECH_DEBT 候选区 F-157 移出 → 处置记录新节（✅ 已修）；TO-TICKETS 归档；AGENTS 状态行追加。
