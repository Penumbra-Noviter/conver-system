...
### 2026-09-22 — 技术债消费批次（F-157 AboutPage 版本防漂移，候选区清零）

> 来源：模拟器简介批次期末全量发现（发布批 7f20557 改版本号漏改 About 页，`git stash` 回基线复现确认）；用户「顺手修掉吧」拍板消费。门禁：全量 **2898 测**绿（含 `manual_pages_test` 版本防漂移测试转绿——基线 2897 存量 1 失败清零）/ `flutter analyze` 0。处置详情见 DEV_LOG〈技术债消费批次 F-157 — AboutPage 版本防漂移〉。

| 编号 | 处置 | 详情 |
|------|------|------|
| F-157 | ✅ 已修 | `AboutPage.appVersion` `'1.0.0+1'` → `'1.1.0+2'`（对齐 pubspec，docstring 同步）；发布手册 `release-android.md` §1「修改位置」**固化双同步点**（pubspec version 行 + AboutPage.appVersion 常量，漏改即防漂移测试红——1.1.0 发布漏改实锤注记）——根治「发版只改 pubspec 漏改 About」复发；commit（见下） |
