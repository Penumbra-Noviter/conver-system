...
|------|--------|------|------|------|----------|
| F-157 | AboutPage 版本显示陈旧：pubspec 已 `1.1.0+2`，AboutPage.appVersion 仍报 `1.0.0+1`（`manual_pages_test` 版本防漂移用例基线即挂，`git stash` 回基线复现确认） | 模拟器简介批次期末全量发现（2026-09-22，非本批引入；发布批 7f20557 改版本号漏改 About 页） | Strong | 📝 待立项 | 发布卫生 |
