...
> 1. 已完成归档完整保留**最近 6 个批次**；更早批次折叠为「历史归档索引」单行（工单号/提交哈希/验收摘要在此保留一条，细节由 git 历史承担：`git log -p -- TO-TICKETS.md`）
...
| Ticket | 标题 | F 项 | 完成日期 | 提交 |
|--------|------|------|----------|------|
| F-139 | CharacterUpdate 加 field_validator 拒绝 name/prompt_mode/expert_prompt 显式 null（防 IntegrityError 500） | F-139 | 2026-09-14 | 47b2eef |
