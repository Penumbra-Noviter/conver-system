...
- 滚动机制（2026-09-21 固化）：最近 **10 个批次节**保留完整，更早折叠为「历史归档索引」（每节一行：日期/批次名/一句话摘要，上限 **100 行**删最旧）；被折叠节原文 `git log -p -- DEV_LOG.md` 可溯。规则正文见 DEV_LOG.md 头部「滚动机制」。
- 只记「已做」与决策/避坑；待办一律进 TO-TICKETS。
