...
- 安装器（仅未加 `-SkipInstaller` 时产出）：`src-tauri/target/release/bundle/nsis/Conver System_0.2.0_x64-setup.exe`（**内含后端随包资源** `dist/conver_backend`，经 `bundle.resources` 分发——安装后即可双击直启，无需 python）
