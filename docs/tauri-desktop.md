...
**POSIX 路径警告**：`CONVER_DATA_DIR` 传 Git Bash 形态的 POSIX 路径（如 `/c/Users/<name>/conver-data`）时**不做归一化**——按逐字符契约（契约表 v2，见上）直接落位，数据将落在字面路径 `C:\c\Users\<name>\conver-data`（而非 `C:\Users\<name>\conver-data`）。Git Bash 环境部署请改用 Windows 风格路径（如 `C:\conver-data`）；代码不做归一化是契约行为，非缺陷。
