...
对 LLM API 做了 CORS 直连实测（preflight curl + 浏览器真实 fetch，假 Key 即可判定——CORS 在鉴权前执行，放行则拿到 4xx、拦截则 TypeError）。证据目录：`.scratch/mobile-cors-spike/`。
...
- 技术可行性事实：[mobile-adaptation-research.md](mobile-adaptation-research.md)
- 模拟器决策：CONSENSUS.md **ADR-0002**（本地托管 + CORS 直连 + 注入/存档桥）；ADR-0001（桌面 key-injector 契约前提）
- 桌面架构参照：[architecture.md](architecture.md)（业务逻辑迁移蓝本）
- 桌面设计 token：`frontend/css/style.css` `:root`
- 桌面 LLM 编排蓝本：[llm-integration.md](llm-integration.md) · `backend/app/services/chat.py`
- 桌面游戏生成/校验蓝本：`backend/app/services/game_generator.py`
- 桌面模拟器适配蓝本：`frontend/js/simulator-adapt.js` · `key-injector.js`
- CORS 实测证据：`.scratch/mobile-cors-spike/`（prototype，请勿复用为生产代码）
