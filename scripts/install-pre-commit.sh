...
# 本库为变体结构：任务池 TO-TICKETS.md、候选区节名「候选区」——钩子固定带对应参数。
...
if ! python "$ROOT/scripts/pool_cleanup_check.py" --check --tickets-file TO-TICKETS.md --candidate-section "## 候选区" 2>&1; then
...
echo "installed: $HOOKS/pre-commit (TO-TICKETS.md + 候选区节名)"