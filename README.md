...
- **模拟器** — 22 款文字模拟器浏览/类型筛选/一键 iframe 运行；AI 驱动游戏可用「重新同步」按钮将主应用接口 Key 同步注入模拟器（SIM-API-1 后自动同步为常态）；「存档管理」面板（导出/导入/删除）可统一管理存档；可导入第三方 `.html` 游戏（仅本地文件 ≤5MB，SHA-256 去重、冲突自动改名，导入后存外置数据目录不随包分发，风险与适配见程序内「导入游戏与安全须知」）
