...
- **对话内模型切换 / 重生成** — 聊天头部模型徽标可随时切换当前对话的 Provider 与模型；对最后一条 AI 回复不满意，可点击「重生成」一键重新生成
- **消息编辑重发 / 删除** — 已发送的用户消息可就地编辑后重新生成后续回复；支持删除单条消息（删除用户消息连带截断其后，删除 AI 消息仅删该条）
- **首启引导与错误提示** — 未配置 AI 接口时聊天区显示引导卡直达设置页；操作失败以可关闭的错误条提示，不再写入消息记录
- **搜索历史消息** — 跨对话关键词搜索，点击结果自动跳转到命中消息并短暂高亮定位
- **本地存储** — 所有数据存于本地 SQLite，无需联网依赖
- **API Key 管理** — 内置设置面板管理密钥
- **模拟器** — 22 款文字模拟器浏览/类型筛选/一键 iframe 运行；AI 驱动游戏可用「重新同步」按钮将主应用接口 Key 同步注入模拟器（SIM-API-1 后自动同步为常态）；导入第三方 `.html` 文件时自动识别游戏类型（AI 驱动/纯本地），误判可点卡片「重新识别」按钮修正；「存档管理」面板（导出/导入/删除）可统一管理存档；可导入第三方 `.html` 游戏（仅本地文件 ≤5MB，SHA-256 去重、冲突自动改名，导入后存外置数据目录不随包分发，风险与适配见程序内「导入游戏与安全须知」）
