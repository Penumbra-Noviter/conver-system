...
测试：`cd backend && python -m pytest`（pytest 413+1skip）；`cd frontend && npm test`（Vitest 551，覆盖率 `npm run test:coverage`）；`cd src-tauri && cargo test`（58）。
...
- ✅ 测试：pytest 413+1skip（后端）+ Vitest 551（前端）+ cargo test 58（壳）全绿
