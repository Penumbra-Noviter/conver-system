...
测试：`cd backend && python -m pytest`（pytest 413+1skip）；`cd frontend && npm test`（Vitest 466，覆盖率 `npm run test:coverage`）；`cd src-tauri && cargo test`（58）。
...
- ✅ 测试：pytest 413+1skip（后端）+ Vitest 466（前端）+ cargo test 58（壳）全绿
