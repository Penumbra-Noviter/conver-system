...
测试：`cd backend && python -m pytest`（pytest 412+1skip）；`cd frontend && npm test`（Vitest 431，覆盖率 `npm run test:coverage`）；`cd src-tauri && cargo test`（56）。
...
- ✅ 测试：pytest 412+1skip（后端）+ Vitest 431（前端）+ cargo test 56（壳）全绿
