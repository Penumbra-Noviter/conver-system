...
测试：`cd backend && python -m pytest`（pytest 809 + 1 skip）；`cd frontend && npm test`（Vitest 1165，覆盖率 `npm run test:coverage`）；`cd src-tauri && cargo test`（70）。权威基线见 [CODE_WIKI.md](CODE_WIKI.md) §5 机械标记。
