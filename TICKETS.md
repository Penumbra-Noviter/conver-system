...
> 当前 2 项待办（T-02 为方案 2 主工单，T-01 并入其批次实现）。

| Ticket | 标题 | 状态 | 验收摘要 |
|--------|------|------|----------|
| T-01 | 模拟器新游戏接入契约：核对脚本 + 覆盖层映射记录结构化 | 📝 已录入 | 见下方工单详情 |
| T-02 | 游戏外置数据目录 + 用户导入 + 安全加固 + 自动适配（方案 2） | 📝 已录入 | 见下方工单详情 |

### T-02 游戏外置数据目录 + 用户导入 + 安全加固 + 自动适配（2026-08-19 录入，Grilling 共识确认）

> 来源：用户咨询「不断引入游戏是否导致 exe 膨胀」→ 方案 2 选定；grill-me 两轮访谈（Q1 单文件低频 / **Q2 分发他人** / Q3 两版过渡 / Q4 导入流程 / Q5 适配边界 + D6-D10 全按推荐 + 静态恶意代码审查讨论）→ 用户确认共识。

**目标**（共识六点）：
1. **外置**：游戏数据目录 `%APPDATA%/ConverSystem/simulators/`（CONVER_DATA_DIR 可覆盖）；首启从内置拷贝 22 款 + manifest（两版过渡：本版仍随包带 22 款，dist 不变、停止线性膨胀；未来删 datas 瘦身）
2. **导入**：列表页「导入游戏」按钮 + 拖拽 .html；校验 .html / ≤5MB / 非空；SHA-256 内容去重（重复提示）+ 文件名冲突自动改名（xxx-2.html）；元数据自动探测（cfg- 前缀输入框 id，探测不到留空走 manifest 条目级降级）
3. **安全**（Q2 分发他人后的升级）：仅本地文件导入（无网络拉取/URL 导入）+ 导入时明确警告（「第三方游戏可读取本地数据并调用 API」）+ 明显恶意模式粗筛提示（eval/document.cookie/跨域 fetch 等，命中弹警告**不拦截**——静态审查不承诺防住，定位知情提示）
4. **自动适配 UI**：共享覆盖层自动注入（现有机制）+ T-01 核对脚本出「未覆盖清单」提示 + per-game CSS 覆盖（数据目录 `<game-id>.css`，注入在共享层之后）
5. **T-01 并入本批次**（核对脚本 = 适配报告核心组件，工单依赖序排在导入流程之前）
6. **文档**：程序内手册补「导入安全须知」

**文件范围（预估）**：backend（数据目录迁移/静态挂载/导入端点/粗筛）、frontend（导入 UI/拖拽/警告/per-game CSS 注入/列表 source 标识）、scripts/check-simulator-css.mjs、docs（手册安全须知）、T-01 相关（simulator-pc.css 映射结构化 + 契约测试）——详细拆票见 plan-tickets 产出。

### T-01 模拟器新游戏接入契约（2026-08-19 录入，用户拍板立项）

> 来源：用户反馈「部分模拟器 UI 仍反人类」→ 诊断结论：覆盖层适配依赖「猜类名/变量」，新游戏接入无强制校验（F2 教训：内层文本 13px 漏盖，靠期末四轴才抓到）。用户确认方案 A 保留（第三方游戏零改动自动生效），补上 A 的短板 = 接入契约。方案 B（_base.css 模板化）暂不切换（第三方游戏不引用 _base.css，收益有限；若未来游戏转为自产再评估切换）。

**目标**：
1. 覆盖层映射记录结构化——simulator-pc.css 中每游戏一行「已核对映射」记录（类名/变量/显式字号覆盖清单），机器可解析
2. 核对脚本 `scripts/check-simulator-css.mjs`——扫描指定游戏 HTML 的日志条目类名 / CSS 变量体系 / 显式字号声明，与覆盖层已覆盖选择器集合比对，输出「未覆盖清单」
3. 接入流程成文——新游戏接入 = 丢文件 + 更新 manifest + 跑核对脚本（红则补映射再复核）

**验收标准（语义契约）**：
- 脚本对现有 22 个游戏全绿（零未覆盖输出，锚：脚本退出码 0 且无「未覆盖」行）
- 证伪用例：构造一个含未覆盖类名（如 `.log-entry .content` 显式 12px 字号）的样本 HTML → 脚本输出该未覆盖项（红）
- 覆盖层映射记录格式可被脚本解析（锚：`# sim-pc:` 标记行 + 每游戏一行）
- CODE_WIKI 或 CLAUDE 记录接入流程（锚：`check-simulator-css` 出现于文档）

**文件范围**：`scripts/check-simulator-css.mjs`（新建）+ `frontend/css/simulator-pc.css`（映射段结构化）+ `CODE_WIKI.md`（接入流程段）
