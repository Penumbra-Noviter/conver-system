...
> 来源：AI风月对标调研五批工单（BR 分支第二张/收官）；新会话消息按快照重建、世界书语义对齐角色级共享模型（Spec 偏差记录见下），规格/契约锁依据见 docs/chat-simulator-upgrade-spec.md §BR-2。叙述详见 DEV_LOG〈BR-2 从快照/分支派生会话（2026-09-11）〉。

| Ticket | 标题 | 完成日期 | 提交 |
|--------|------|----------|------|
| BR-2 | 从快照/分支派生会话（clone_conversation / branch_from_message + 三路由 + 删源置空） | 2026-09-11 | 90ea7b4 |

**验证链：** 后端 pytest 965+1skip→983+1skip（+18：test_conversation_branch——clone 往返一致（消息/候选/激活/created_at 保真）/clone 防御矩阵（未知角色 404、非法角色、激活越界、content≠激活候选即拒）/分支逐条一致 + 父/锚/分支名记录/源零改动/世界书不重插不污染 + 分支与源视角一致/删源置空不连坐/路由 404 与版本拒绝/快照下载头）| api-design 补 MS-2/MS-3/BR-2 端点文档（补此前缺口）| 全量后端绿零回归（前端/cargo 零改动）| 期末四轴：Spec 1 偏差记录（「世界书条目独立可改」在角色级共享模型下不可兑现——重插会造成角色条目重复副本破坏记忆宫殿增量计数不变量；契约锁 #3 改为锁定不重插不污染 + 共享视角一致）+ Standards 0 硬违例（conversation_export 延迟导入防循环）| Falsify 0 HIGH（防御矩阵全锁）| doc_sync 6 标记刷新零漂移
**非阻断落债：** 无（BR 批收官；前端分支 UI——消息级「分支」操作 + 会话列表分支来源标记 + 模拟器「以此存档开新对话」——未消费，spec §BR-2 前端段留待立项）
...
> 来源：AI风月对标调研五批工单（MS 消息操作第二张）；候选控制条承接 MS-1 content 跟随激活模型，规格/契约锁依据见 docs/chat-simulator-upgrade-spec.md §MS-2。叙述详见 DEV_LOG〈MS-2 swipes 前端（2026-09-10）〉。

| Ticket | 标题 | 完成日期 | 提交 |
|--------|------|----------|------|
| MS-2 | swipes 前端（候选计数 + 左右切换 + 失败回滚；重生成改为追加候选） | 2026-09-10 | ef6be24 |

**验证链：** 后端 pytest 923+1skip（+1：test_message_swipes 路由用例——消息列表含候选集/激活序号、switch-swipe 端点切换生效/越界 400）| 前端 Vitest 1213→1220（+7：chat-swipes 契约锁——计数渲染/单选与无 swipes 不渲染/切换调用参数与乐观更新/边界不越界/失败回滚/重叠切换并发保护）| 全量双端绿零回归 | 期末 code-review：Standards 0 硬违例 / Falsify 2 项当场修复（消息列表 N+1 查询 → _with_swipes_batch 批量填充；重叠切换竞态 → generation token 仅最后一次调用失败才回滚）+ Standards 低危（退化断言 → 真实调用参数断言）| doc_sync 零漂移
**非阻断落债：** 无
