...
> 里程碑 M1–M7 待推进（M0 已交付归档，见下）。M5 模拟器单里程碑偏大，开工前用 /to-tickets 再细分（本地服务器 → 注入 → 存档桥 → 导入/生成），拆出的子项按 F 编号展开。
...
> 已迁移至独立文件 [TECH_DEBT.md](TECH_DEBT.md)——当前 **6 项待立项**（F-1~F-6，M0 kickoff 批次审核落盘，全部非阻断）。
