...
协议表面（__all__）：MAX_RETRIES / ScanResult / generate_game / scan_generated_html / validate_generated_html
"""
...
class GenerateResult:
    ...
    ok: bool
    game: dict | None = None
    errors: list[GenValidationError] | None = None
...
def _check_html_structure(html: str) -> GenValidationError | None:
    """检查 1：HTML 骨架完整性（含 <!DOCTYPE html> 或 <html>）"""
    lower = html.lower().strip()
    if lower.startswith("<!doctype html") or "<html" in lower[:200]:
        return None
    return GenValidationError(field="structure", message="生成的 HTML 缺少 <!DOCTYPE html> 或 <html> 标签")


def _check_template_completeness(html: str) -> GenValidationError | None:
    """检查 2：模板标记完整性（无剩余 <!-- GEN: 标记）"""
    if MARKER_PATTERN.search(html):
        return GenValidationError(field="template", message="模板标记未完全填充（仍有 <!-- GEN: --> 未替换）")
    return None


def _check_cfg_contract(html: str) -> GenValidationError | None:
    ...
    if missing:
        ...
        return GenValidationError(field="cfg", message=f"缺少 AI 配置输入框：{names}")
    return None
...
def _check_html_parseability(html: str) -> GenValidationError | None:
    ...
    try:
        ...
        return GenValidationError(field="syntax", message=f"HTML 语法错误：{exc}")
    if checker.errors:
        return GenValidationError(field="syntax", message=f"HTML 解析警告：{'；'.join(checker.errors[:3])}")
    return None


def _check_security(html: str, precomputed_warnings: list[str] | None = None) -> list[GenValidationError]:
    ...
    if warnings:
        return [GenValidationError(field="security", message=f"检测到可疑模式：{'、'.join(warnings)}")]
    return []
...
def _check_game_data(html: str) -> GenValidationError | None:
    """检查 6：游戏数据有效性（场景 JSON 可解析、至少一个场景、每场景有叙事与 choices、
    id 唯一、选项引用完整）"""
    scenes, error = _try_extract_scenes(html)
    if error:
        return GenValidationError(field="data", message=error)
    return None
...
def validate_generated_html(
    ...
    *,
    ...
    Returns:
        错误列表（空列表表示全部通过）
    """
    errors: list[GenValidationError] = []
...
def _build_retry_prompt(description: str, title: str | None,
                        errors: list[GenValidationError], suggestion: str,
...
async def _generate_with_retry(
    ...
    *,
    ...
    previous_errors: list[GenValidationError] | None = None,
    ...
    if not isinstance(reply, str):
        ...
        if retries_left <= 0:
            return GenerateResult(
                ok=False,
                errors=[GenValidationError(field="data", message=f"LLM 返回了非字符串类型：{type(reply).__name__}")],
                suggestion="请重试",
                retries=attempted + 1,
            )
        return await _generate_with_retry(
            ...
            previous_errors=[GenValidationError(field="data", message=f"LLM 返回非字符串类型：{type(reply).__name__}")],
...
def _build_suggestion(errors: list[GenValidationError]) -> str:
    """根据校验错误生成修正建议"""
    suggestions = []
    for err in errors:
