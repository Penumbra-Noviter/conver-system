...
- LLM 族：委托 services/error_mapping.py::llm_error_response 单一入口
  （401/429/504/400/502；防御性注册——请求路径上的 LLM 错误实际先经
  complete_chat 显式 raise HTTPException 并携带 provider 上下文）
...
async def llm_error_handler(request: Request, exc: LLMError) -> JSONResponse:
    """LLM 异常统一映射（401/429/504/400/502，经 services/error_mapping.py::llm_error_response）
    ...
    Returns:
        携带状态码与 detail 的 JSON 响应
    """
    status_code, message = llm_error_response(exc, None)
