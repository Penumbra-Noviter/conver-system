...
> 生成日期：2026-08-15
> 测试状态：<!--AUTO:tests_total:total-->2898<!--/AUTO--> 项全绿（pytest <!--AUTO:tests_total:pytest-->1356<!--/AUTO--> + Vitest <!--AUTO:tests_total:vitest-->1472<!--/AUTO--> + cargo test <!--AUTO:tests_total:cargo-->70<!--/AUTO-->）
>
...
### 4.8 `backend/app/api/routes/conversations.py` — 会话路由（<!--AUTO:lines:backend/app/api/routes/conversations.py-->~160 行<!--/AUTO-->）
...
### 4.14 `backend/app/services/chat.py` — 对话编排（<!--AUTO:lines:backend/app/services/chat.py-->~993 行<!--/AUTO-->）
...
### 4.20 `backend/app/services/exceptions.py` — 领域异常（<!--AUTO:lines:backend/app/services/exceptions.py-->~73 行<!--/AUTO-->）

**职责**：领域异常定义（404/409/422 类），供 service 层抛出、errors.py 统一处理。

> 无公开函数（异常类层次）。

### 4.21 `backend/app/services/message.py` — 消息服务（<!--AUTO:lines:backend/app/services/message.py-->~457 行<!--/AUTO-->）
...
| <!--AUTO:sig:backend/app/services/message.py:build_message_list-->`build_message_list(db, conversation, user_content, max_rounds=30, user_name='User', append_current_input=True, world_injection=None, history=None, narrative_style='', preset_dialogue='')`<!--/AUTO--> | 构建 LLM 上下文（滑窗 + 模板变量） |
...
### 4.21.6 `backend/app/services/lorebook_engine.py` — 世界书激活引擎（WL-2）（<!--AUTO:lines:backend/app/services/lorebook_engine.py-->~208 行<!--/AUTO-->）
...
| 元素 | 说明 |
|------|------|
| <!--AUTO:sig:backend/app/services/lorebook_engine.py:activate_lorebook_entries-->`activate_lorebook_entries(entries, scan_text, *, current_input='', rng=None)`<!--/AUTO--> | 命中判定 + 概率/互斥组 + 排序主入口 |
| <!--AUTO:sig:backend/app/services/lorebook_engine.py:build_world_injection-->`build_world_injection(activated, *, user_name='User', char_name='Character', source_by_id=None)`<!--/AUTO--> | position 分组（world→system）构建注入块，{{user}}/{{char}} 模板替换 |
...
### 4.21.8 `backend/app/services/memory_palace.py` — 记忆宫殿（WL-5）（<!--AUTO:lines:backend/app/services/memory_palace.py-->~213 行<!--/AUTO-->）
...
| <!--AUTO:sig:backend/app/services/memory_palace.py:summarize_turn-->`summarize_turn(history, *, provider, model=None, user_name='User', char_name='Character', extra_instructions='')`<!--/AUTO--> | 归纳 → MemoryDraft（剥围栏 + 严格 JSON；失败降级 None 不抛） |
...
### 4.21.12 `backend/app/services/mods.py` — Mod 挂载层（MD-1）（<!--AUTO:lines:backend/app/services/mods.py-->~377 行<!--/AUTO-->）
...
| <!--AUTO:sig:backend/app/services/mods.py:list_enabled_mods_for_area-->`list_enabled_mods_for_area(db, character_id, target_area)`<!--/AUTO--> | 读取角色启用且 target_area 匹配的 Mod（ModPayload，按 (sort_order, mod_id) 升序，IN 防 N+1） |
...
### 4.23 `backend/app/services/setting.py` — 设置服务（<!--AUTO:lines:backend/app/services/setting.py-->~241 行<!--/AUTO-->）
...
### 4.29 `backend/app/services/llm/prompt.py` — 提示词构建（<!--AUTO:lines:backend/app/services/llm/prompt.py-->~358 行<!--/AUTO-->）
...
| <!--AUTO:sig:backend/app/services/llm/prompt.py:build_messages-->`build_messages(character, history, user_content, max_rounds=30, user_name='User', append_current_input=True, world=None, narrative_style='', preset_dialogue='')`<!--/AUTO--> | 组装完整消息序列（system + 滑窗历史 + 当前） |
...
### 4.33 `frontend/js/api.js` — 统一请求层（<!--AUTO:lines:frontend/js/api.js-->~439 行<!--/AUTO-->）
...
### 4.34 `frontend/js/app.js` — 应用编排（<!--AUTO:lines:frontend/js/app.js-->~426 行<!--/AUTO-->）
...
### 4.36 `frontend/js/chat.js` — 对话视图（<!--AUTO:lines:frontend/js/chat.js-->~1348 行<!--/AUTO-->）
...
| <!--AUTO:sig:frontend/js/chat.js:branchLastReply-->`branchLastReply()`<!--/AUTO--> | F-100 末条 assistant 分支（POST branch → 刷新 + activateConversation 创建即打开） |
...
### 4.36.5 `frontend/js/list-views.js` — 角色/对话列表视图（<!--AUTO:lines:frontend/js/list-views.js-->~567 行<!--/AUTO-->）
...
### 4.36.6 `frontend/js/error-bar.js` — 错误条深模块（<!--AUTO:lines:frontend/js/error-bar.js-->~134 行<!--/AUTO-->）
...
### 4.37 `frontend/js/components/character-form.js` — 角色编辑表单（<!--AUTO:lines:frontend/js/components/character-form.js-->~378 行<!--/AUTO-->）
...
### 4.38 `frontend/js/components/character-submit.js` — 提交状态机（<!--AUTO:lines:frontend/js/components/character-submit.js-->~331 行<!--/AUTO-->）
...
### 4.39 `frontend/js/components/character-wizard.js` — 六步创建向导（<!--AUTO:lines:frontend/js/components/character-wizard.js-->~685 行<!--/AUTO-->）
...
### 4.44 `frontend/js/components/settings-panel.js` — 设置面板（<!--AUTO:lines:frontend/js/components/settings-panel.js-->~461 行<!--/AUTO-->）
...
### 4.44.0 `frontend/js/components/prompt-debug.js` — Prompt Debug 只读预览面板（PD-4）（<!--AUTO:lines:frontend/js/components/prompt-debug.js-->~167 行<!--/AUTO-->）

**职责**：只读 Prompt Debug 面板——`showPromptDebug(conversationId)` 入口（复用 openModal 骨架 + 幂等去重）+ 分段渲染（role 徽标 + 来源色标 SOURCE_CLASS 单一映射表 + content escapeHtml）+ 空态/错误态。样式自包含 `<style>` 内嵌（对齐 mod-css.js 先例）。入口：chat.js 头部 #btn-prompt-debug。

| 元素 | 说明 |
|------|------|
| <!--AUTO:sig:frontend/js/components/prompt-debug.js:showPromptDebug-->`showPromptDebug(conversationId)`<!--/AUTO--> | 打开只读面板（拉取 + openModal + 失败错误态 + 幂等去重） |
| <!--AUTO:sig:frontend/js/components/prompt-debug.js:sourceClass-->`sourceClass(source)`<!--/AUTO--> | 来源 → 色标类名（非法值回落 SOURCE_DEFAULT_CLASS） |
| <!--AUTO:sig:frontend/js/components/prompt-debug.js:segmentsHtml-->`segmentsHtml(segments)`<!--/AUTO--> | 分段列表渲染（空/非数组 → 空态） |
| <!--AUTO:sig:frontend/js/components/prompt-debug.js:promptDebugBodyHtml-->`promptDebugBodyHtml(data)`<!--/AUTO--> | 面板 body 渲染（meta 三字段 + 分段转义） |
...
### 4.49 `frontend/js/format.js` — 展示契约（<!--AUTO:lines:frontend/js/format.js-->~283 行<!--/AUTO-->）
...
### 4.50 `frontend/js/icons.js` — 图标 seam（<!--AUTO:lines:frontend/js/icons.js-->~68 行<!--/AUTO-->）
...
| <!--AUTO:sig:frontend/js/key-injector.js:observeConfigControls-->`observeConfigControls(params = {})`<!--/AUTO--> | 配置控件观察（观察者生命周期收口，S3） |
...
### 4.56 `frontend/js/simulator-view.js` — 模拟器运行视图（<!--AUTO:lines:frontend/js/simulator-view.js-->~405 行<!--/AUTO-->）
...
| <!--AUTO:sig:frontend/js/stream-session.js:settleTurn-->`settleTurn({ convId, getTab, updateTab, isActive, render, revision, settleIndex = -1, anchor = null, messageId = null, content = '', replaceId = null })`<!--/AUTO--> | 统一结算（完成/停止/出错写回） |
| <!--AUTO:sig:frontend/js/stream-session.js:settleByPosition-->`settleByPosition(tab, anchor, message, replaceId = null)`<!--/AUTO--> | 按位置结算 |
| <!--AUTO:sig:frontend/js/stream-session.js:mergeFreshList-->`mergeFreshList(tab, revision, msgs, { settleIndex = -1, anchor = null, messageId = null, content = '', replaceId = null } = {})`<!--/AUTO--> | 合并刷新消息列表 |
...
三层测试体系：后端 pytest（50 文件）、前端 Vitest（44 文件）、壳 cargo test（4 集成文件 + lib.rs 单元）。覆盖率基线：后端 `pytest --cov`（目标 ≥90%）、前端 `npm run test:coverage`。
...
| `backend/tests/test_gallery.py` | <!--AUTO:tests:backend/tests/test_gallery.py-->45<!--/AUTO--> | CG 资产库契约锁（CG-2：入库去重同作品同 url/解锁幂等/加权抽选同种子顺序无关与分布/SET NULL 会话删图留/CASCADE 作品删图清/404 守卫） |
...
| `backend/tests/test_cg_auto_trigger.py` | <!--AUTO:tests:backend/tests/test_cg_auto_trigger.py-->16<!--/AUTO--> | CG 自动触发契约锁（T6：概率判定/候选池过滤/锚定/隔离/双路径接线/settings 键读写） |
...
| `backend/tests/test_conversation_greeting.py` | <!--AUTO:tests:backend/tests/test_conversation_greeting.py-->7<!--/AUTO--> | 预设开场白 override 契约锁（PD-1：greeting 四态/模板变量/无开场白） |
...
| `backend/tests/test_narrative_style_injection.py` | <!--AUTO:tests:backend/tests/test_narrative_style_injection.py-->15<!--/AUTO--> | 叙述风格 prompt 注入契约锁（NPD-02：注入序/空值零注入/expert/prompt-debug source） |
...
| `backend/tests/test_preset_dialogue_injection.py` | <!--AUTO:tests:backend/tests/test_preset_dialogue_injection.py-->15<!--/AUTO--> | 预设对话 few-shot 注入契约锁（NPD-06：快照语义/注入序/与叙述风格同开序正确） |
...
| `backend/tests/test_lorebook_engine.py` | <!--AUTO:tests:backend/tests/test_lorebook_engine.py-->29<!--/AUTO--> | 世界书激活引擎纯函数契约锁（WL-2：命中矩阵/大小写/depth 边界/概率 RNG 复现/互斥组/排序/零 DB 导入） |
...
| `backend/tests/test_mods.py` | <!--AUTO:tests:backend/tests/test_mods.py-->45<!--/AUTO--> | Mod 挂载层契约锁（MD-1：绑定唯一/禁用零影响/sort_order 叠加序含同序稳定/解绑与级联/空列表零变化/CRUD/404 守卫/target_area 过滤/payload 容错） |
...
| `backend/tests/test_prompt_debug.py` | <!--AUTO:tests:backend/tests/test_prompt_debug.py-->11<!--/AUTO--> | Prompt Debug 契约锁（PD-3：来源标注/零变化/空态/expert） |
...
| `backend/tests/test_regenerate.py` | <!--AUTO:tests:backend/tests/test_regenerate.py-->25<!--/AUTO--> | regenerate 端点（截断/事务/错误矩阵） |
...
| `frontend/tests/expert-mode.test.js` | <!--AUTO:tests:frontend/tests/expert-mode.test.js-->21<!--/AUTO--> | 专家模式两态编辑契约锁（PD-6：切换/生成/保存 payload/还原） |
...
| `frontend/tests/format.test.js` | <!--AUTO:tests:frontend/tests/format.test.js-->68<!--/AUTO--> | 展示契约 |
| `frontend/tests/greeting-editor.test.js` | <!--AUTO:tests:frontend/tests/greeting-editor.test.js-->47<!--/AUTO--> | 备用开场白契约锁（PD-2：增删上限/归一/payload/下拉映射） |
...
| `frontend/tests/prompt-debug.test.js` | <!--AUTO:tests:frontend/tests/prompt-debug.test.js-->18<!--/AUTO--> | Prompt Debug 面板契约锁（PD-4：色标映射/转义/空态/非法来源回落） |
...
| `frontend/tests/settings-panel.test.js` | <!--AUTO:tests:frontend/tests/settings-panel.test.js-->39<!--/AUTO--> | 设置面板 |
...
> 三层合计：**<!--AUTO:tests_total:total-->2898<!--/AUTO-->** 项全绿。
>
> - pytest（后端，含 1 skip）：<!--AUTO:tests_total:pytest-->1356<!--/AUTO-->
