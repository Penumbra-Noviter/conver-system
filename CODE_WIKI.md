...
> 生成日期：2026-08-15
> 测试状态：<!--AUTO:tests_total:total-->1754<!--/AUTO--> 项全绿（pytest <!--AUTO:tests_total:pytest-->705<!--/AUTO--> + Vitest <!--AUTO:tests_total:vitest-->979<!--/AUTO--> + cargo test <!--AUTO:tests_total:cargo-->70<!--/AUTO-->）
...
### 4.3 `backend/app/database.py` — 引擎与会话（<!--AUTO:lines:backend/app/database.py-->~45 行<!--/AUTO-->）
...
### 4.8 `backend/app/api/routes/conversations.py` — 会话路由（<!--AUTO:lines:backend/app/api/routes/conversations.py-->~87 行<!--/AUTO-->）

**职责**：会话 CRUD + 清空 + JSON/Markdown 导出端点。
...
### 4.16 `backend/app/services/conversation_export.py` — 会话导出（<!--AUTO:lines:backend/app/services/conversation_export.py-->~119 行<!--/AUTO-->）
...
| <!--AUTO:sig:backend/app/services/message.py:create_message-->`create_message(db, conversation_id, role, content)`<!--/AUTO--> | 写入消息 |
...
| <!--AUTO:sig:frontend/js/simulators.js:bindEvents-->`bindEvents()`<!--/AUTO--> | 事件绑定 |
| <!--AUTO:sig:frontend/js/simulators.js:renderShell-->`renderShell()`<!--/AUTO--> | 渲染壳 |
| <!--AUTO:sig:frontend/js/simulators.js:renderList-->`renderList()`<!--/AUTO--> | 渲染列表 |
...
### 4.74.1 `backend/app/services/simulator_manifest.py` — manifest 读写工具（<!--AUTO:lines:backend/app/services/simulator_manifest.py-->~109 行<!--/AUTO-->）

**职责**：T-02 从 `simulator_store` 拆分。manifest.json 读取/原子写入/追加（缺失/损坏自愈重建）。stdlib only。
...
| `ScanResult` | 预计算扫描结果 dataclass（game_type/config/warnings，T-03） |
...
| <!--AUTO:sig:backend/app/services/simulator_import.py:probe_config-->`probe_config(html_text)`<!--/AUTO--> | cfg- 三元组探测（齐全 → ai+config；否则 local 无 config 降级） |
...
| <!--AUTO:sig:backend/app/services/simulator_import.py:scan_input_ids-->`scan_input_ids(html_text)`<!--/AUTO--> | 输入 id 扫描（cfg- 三元组 + 输入控件 id 收集） |
...
三层测试体系：后端 pytest（20 文件）、前端 Vitest（27 文件）、壳 cargo test（4 集成文件 + lib.rs 单元）。覆盖率基线：后端 `pytest --cov`（目标 ≥90%）、前端 `npm run test:coverage`。
...
> 三层合计：**<!--AUTO:tests_total:total-->1754<!--/AUTO-->** 项全绿。
>
